...
class GCN(object):
    ...
    def __init__(self, graph, learning_rate=0.01, epochs=200,
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % checkpoint_steps == 0:
                saver.save(self.sess, checkpoint_dir + 'model.ckpt', global_step=epoch + 1)
    ...
    def predict(self, testGraph):
        meta_path = '../model/model.ckpt-200.meta'
        ...
        with tf.Session(config=config) as sess:
            ...
            self.preprocess_testdata(testGraph)
            self.build_testplaceholders()
            self.build_testdata(testGraph)
            # Construct feed dictionary
            feed_dict = dict()
            feed_dict.update({self.testplaceholder['features']: self.testfeatures})
            feed_dict.update({self.testplaceholder['support'][i]: self.testsupport[i] for i in range(len(self.testsupport))})
            feed_dict.update({self.testplaceholder['num_features_nonzero']: self.testfeatures[1].shape})
            feed_dict.update({self.placeholders['labels_mask']: self.testdata_mask})

            sess.run([self.model.predict()], feed_dict=feed_dict)

            prob_op = graph.get_operation_by_name('softmax_tensor')
    ...
    def build_testplaceholders(self):
        num_supports = 1
    ...
    def build_testdata(self,testgraph):
        g = testgraph.G
        shuffle_indices = np.random.permutation(np.arange(g.number_of_nodes()))
        def sample_mask(begin, end):
            mask = np.zeros(g.number_of_nodes())
            for i in range(begin, end):
                mask[shuffle_indices[i]] = 1
            return mask
        test_size = int(g.number_of_nodes())
        self.testdata_mask = sample_mask(0, test_size)
    ...
    def preprocess_testdata(self,graph):
        """
            x_test
        """
        g = graph.G
        look_back = graph.look_back_list
        self.testfeatures = np.vstack([g.nodes[look_back[i]]['feature']
            for i in range(g.number_of_nodes())])
        self.testfeatures = preprocess_features(self.testfeatures)
        adj = nx.adjacency_matrix(g) # the type of graph
        self.testsupport = [preprocess_adj(adj)]
