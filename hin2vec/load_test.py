...
def main(graph_fname, node_vec_fname, path_vec_fname, options):
    ...
    print ('Load a HIN...')
    g = loader.load_a_HIN_from_edge_file(graph_fname)
    print(g.node2id)
...
def output_path2vec(g, tmp_path_vec_fname, path_vec_fname):
    with open(tmp_path_vec_fname) as f:
        with open(path_vec_fname, 'w') as fo:
            id2edge_class = dict([(v, k) for k, v in g.edge_class2id.items()])
            linenum = 0
            for line in f:
                linenum += 1
                if linenum==1:
                    ...
                    for id in ids:
                        if id==",":
                            ids.remove(",")
                    edge = ','.join([id2edge_class[int(id_)] for id_ in list(ids)])
                    line = '%s %s\n' % (edge, vectors)
                    fo.write(line)
...
if __name__ == '__main__':
    ...
    parser.add_option('-w', '--window', action='store', dest='window',
                      default=3, type='int',
                      help='Max window length (default: 3)，关心几度')
    ...
    options, args = parser.parse_args('./res/load_heterogeneous.csv node_vectors.txt metapath_vectors.txt -l 500 -d 20 -w 4'.split())
    print(options)
    print(args)
    if len(args) != 3:
        parser.print_help()
